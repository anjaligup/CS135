...
(define (distinct-elems? tuple)
  (if (equal? (length(make-set tuple)) (length tuple))
      #t
      #f))
...
(define (rcons x object)
  (if (equal? (list? object) #f) (list x object)
      (insert x object)))
  


;; The below function should take an object x as an argument and a list lst = (e1 ..., en).
...
(define (helper x lst result)
  (cond [(null? lst) (reverse result)]
        [(list? (car lst)) (helper x (cdr lst) (cons(cons x (car lst)) result))]
        [else (helper x (cdr lst) (cons (cons x (list (car lst))) result))]))
...
(define (cartesian-product list-of-lists)
  (cond [(null? (car list-of-lists))
         '()]
        [(or (equal? (length list-of-lists) 1) (equal? (length list-of-lists) 0))
         '()]
        [(equal? (length list-of-lists) 2 ) (cartesian-product2 (car list-of-lists) (car (cdr list-of-lists)))]
...
(define (subsets k lst)
  ...
  (define list-of-lists
    (if (zero? k)
        '()
        (cons lst (list-of-lists lst (- k 1))))
    )
  ...
  (define subsets-with-permutations
    (filter (lambda (tuple) (eq? (length (make-set tuple)) k)) cart-prod))
  ...
  (define repeated-tuples
    (map order subsets-with-permutations)
    )
