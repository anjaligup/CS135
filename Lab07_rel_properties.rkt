...
;;-------------------------------------------------------------------------------
;; Name: Anjali Gupta
;; Pledge: I pledge my honor that I have abided by the Stevens Honor Syste - AG
;;-------------------------------------------------------------------------------
...
(define (id n)
  (define (id-build curr result)
    (if (> curr n)
        result
        (id-build (+ curr 1)(append result (list (list curr curr))))))
  (let ((id-set (id-build 1 '())))
    (make-set id-set)))
...
(define (relates-to v R)
  (cond[(null? R) '()] [(equal? v (car (car R))) (append (cdr (car R)) (relates-to v (cdr R)))]
