...
(define (subset? SA SB)
  (if (equal? (length SA) (length(intersection SA SB))) #t #f))
...
(define (set-equal? SA SB)
  (equal? (make-set (append SA SB)) SB))
...
(define (set-difference LA LB)
  (make-set (if (null? LA)
                '()
            (if (element? (car LA)(intersection LA LB))
...
(define (remove e S)
  (make-set (if (element? e S)
                (set-difference S (cons e '()))
                S)))
