...
(define (nth n lst)
  (if (zero? n) ;;if first index 0, return first element of list
      (car lst)
      (nth (- n 1) (cdr lst)))) ;;recursively call function and decerement n and chop off first element
...
(define (sum lst)
  (if (null? lst) ;; list empty, return 0
      0
      (+ (car lst) (sum (cdr lst))))) ;; add first element to rest of list
...
(define (product lst)
  (if (null? lst) ;; if list is empty, return 1
      1
...
(define (filter pred lst)
  (if (null? lst)
      `()
   (if (pred (car lst)) ;;if evaluates to true
       (cons (car lst)(filter pred (cdr lst))) ;;cons that element to the rest of the list
       (filter pred (cdr lst))))) ;; else recursively call function to remainder of list
